...
struct CostUsageCodexRowDedupTests {
    ...
    func `re-emitted copies deduplicate only when canonical totals prove them redundant`() {
        ...
        let canonical = [day: ["gpt-5.2": [1100, 900, 110]]]
    ...
    func `distinct events sharing content are kept when canonical totals count them all`() {
        ...
        let canonical = [day: ["gpt-5.2": [2000, 1800, 200]]]
    ...
    func `groups that cannot land on canonical totals stay verbatim`() {
        ...
        let canonical = [day: ["gpt-5.2": [1500, 1350, 150]]]
    ...
    func `deduplication keeps the copy with the most pricing evidence`() throws {
        ...
        let canonical = [day: ["gpt-5.2": [1000, 900, 100]]]
    ...
    @Test
    func `replay retains observed priority pricing after trace pruning`() throws {
        let rows = [
            self.row(eventIndex: 0, timestampUnixMs: 1000, pricingMode: "priority"),
            self.row(eventIndex: 5, timestampUnixMs: 1000, pricingMode: "standard"),
            self.row(eventIndex: 6, timestampUnixMs: 2000, input: 2000, pricingMode: "standard"),
            self.row(eventIndex: 9, timestampUnixMs: 2000, input: 2000, pricingMode: "standard"),
        ]
        let repaired = CostUsageScanner.deduplicatedCodexUsageRows(
            rows, canonicalDays: ["2026-09-13": ["gpt-5.2": [3000, 1800, 200]]])

        #expect(repaired.count == 2)
        #expect(try #require(repaired.first { $0.timestampUnixMs == 1000 }).pricingMode == "priority")
    }

    @Test
    func `token equality cannot discard separate zero-token charges`() {
        let rows = [
            self.row(eventIndex: 0),
            self.row(eventIndex: 1),
            self.row(eventIndex: 2, input: 0, cached: 0, output: 0, knownCostNanos: 1_000_000_000),
            self.row(eventIndex: 3, input: 0, cached: 0, output: 0, knownCostNanos: 2_000_000_000),
        ]
        let repaired = CostUsageScanner.deduplicatedCodexUsageRows(
            rows, canonicalDays: ["2026-09-13": ["gpt-5.2": [1000, 900, 100]]])

        #expect(repaired.count == 3)
        #expect(repaired.compactMap(\.knownCostNanos).reduce(0, +) == 3_000_000_000)
    }

    @Test
    func `replay cannot erase an unpriced usage marker`() {
        let rows = [
            self.row(eventIndex: 0, unpricedTokens: 1100),
            self.row(eventIndex: 5),
        ]
        let repaired = CostUsageScanner.deduplicatedCodexUsageRows(
            rows, canonicalDays: ["2026-09-13": ["gpt-5.2": [1000, 900, 100]]])
    ...
    func `canonical pricing rows resolve a group inflated by re-emitted copies`() {
        ...
        let usage = CostUsageScanner.makeFileUsage(
            mtimeUnixMs: 1,
            size: 1,
            days: [day: ["gpt-5.2": [1500, 1200, 150]]],
            parsedBytes: 1,
            codexRows: rows,
    ...
    func `canonical pricing rows still reject groups the canonical totals cannot explain`() {
        ...
        #expect(reconciled.unresolvedGroups == [CostUsageScanner.CodexDayModelKey(day: day, model: "gpt-5.2")])
    }
    ...
    func `store reload drops persisted re-emitted rows and the day keeps its cost`() throws {
        ...
        let usage = CostUsageScanner.makeFileUsage(
            ...
            size: 1,
            days: [range.sinceKey: ["gpt-5.2": [2000, 1800, 200]]],
            parsedBytes: 1,
            codexRows: persisted,
            codexScanComplete: true)
    ...
    func `repaired rows persist even when the unchanged-content shortcut applies`() async throws {
        ...
        var cache = Self.dedupSeedCache(
            path: path,
            rows: persisted,
            days: [range.sinceKey: ["gpt-5.2": [1500, 1200, 150]]],
            range: range)
        _ = CostUsageStoreAccess.replace(cacheRoot: environment.cacheRoot, cache: cache, calendar: range.calendar)
    ...
    func `rows appended after a repair survive save and reload`() async throws {
        ...
        // The planner must compare the repaired baseline with the persisted count instead
        // of appending from the pre-repair count and silently dropping both new rows.
        ...
        usage.days = [range.sinceKey: ["gpt-5.2": [2000, 1500, 200]]]
